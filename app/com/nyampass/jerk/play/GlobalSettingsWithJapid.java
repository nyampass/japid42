...
import com.nyampass.jerk.play.routing.JaxrsRouter;
		...
		if (!cacheResponse) {
			return new Action.Simple() {
				public Promise<SimpleResult> call(Context ctx) throws Throwable {
					// pass the FQN to the jerk controller to determine the
					// template to use
		...
		return new Action<Cached>() {
			public Promise<SimpleResult> call(Context ctx) {
				try {
					...
					if (result == null) {
						// pass the action name hint to jerk controller
						threadData.put(ACTION_METHOD, actionName);
...
//				JapidFlags.debug("route with Japid router");
	...
	// String property = _app.configuration().getString("jerk.dump.request");
	...
	/**
	 * set the folder to save the jerk classes cache file
	 * 
