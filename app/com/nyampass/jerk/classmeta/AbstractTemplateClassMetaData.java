...
package com.nyampass.jerk.classmeta;
...
//import com.nyampass.jerk.compiler.Tag;
import com.nyampass.jerk.compiler.Tag.TagDef;
import com.nyampass.jerk.template.ActionRunner;
import com.nyampass.jerk.template.JapidTemplateBaseStreaming;
import com.nyampass.jerk.template.JapidTemplateBaseWithoutPlay;
import com.nyampass.jerk.template.RenderResult;
import com.nyampass.jerk.template.RenderResultPartial;
...
public abstract class AbstractTemplateClassMetaData {
	...
	public void printHeaders() {
		...
		if (streaming)
			pln(IMPORT_SPACE + com.nyampass.jerk.tags.streaming.Each.class.getName() + COMMA);
		else
			pln(IMPORT_SPACE + com.nyampass.jerk.tags.Each.class.getName() + COMMA);
...
//		JapidFlags.debug("wrote version tag to Java derives: " + v );
	...
	private boolean considerPlayDependency(String l) {
		...
		if (l.startsWith("com.nyampass.jerk.play"))
			return false;
...
//				superClass = JapidTemplateBase.class.getName();
				superClass = "com.nyampass.jerk.play.JapidTemplateBase";
	...
	protected void processDefTags() {
		for (TagDef tag : this.defTags) {
			...
			pln("this.setOut(sb);");
			if (useWithPlay)
				pln("TreeMap<Integer, com.nyampass.jerk.template.ActionRunner> parentActionRunners = actionRunners;\n" +
			...
			if (useWithPlay)
				pln("if (actionRunners.size() > 0) {\n" + 
						...
						"	int segStart = 0;\n" + 
						"	for (Map.Entry<Integer, com.nyampass.jerk.template.ActionRunner> _arEntry : actionRunners.entrySet()) {\n" +
						"		int pos = _arEntry.getKey();\n" + 
						"		_sb2.append(sb.substring(segStart, pos));\n" + 
						"		segStart = pos;\n" + 
						"		com.nyampass.jerk.template.ActionRunner _a_ = _arEntry.getValue();\n" +
